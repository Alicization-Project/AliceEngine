...
__device__
__forceinline double erand48(unsigned short xseed[3])
{
	_dorand48(xseed);
...
struct Vec {        // Usage: time ./smallpt 5000 && xv image.ppm
	...
	__device__ Vec(double x_ = 0, double y_ = 0, double z_ = 0) { x = x_; y = y_; z = z_; }
	__device__ Vec operator+(const Vec &b) const { return Vec(x + b.x, y + b.y, z + b.z); }
	__device__ Vec operator-(const Vec &b) const { return Vec(x - b.x, y - b.y, z - b.z); }
	__device__ Vec operator*(double b) const { return Vec(x*b, y*b, z*b); }
